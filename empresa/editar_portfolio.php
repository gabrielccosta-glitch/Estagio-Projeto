...
if ($_SESSION['tipo_usuario'] == 'admin') {
    $sql = "
        SELECT p.*, e.id AS empresa_id, e.nome_empresa
        FROM portfolio p
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['portfolio_imagem']) && $_FILES['portfolio_imagem']['error'] == UPLOAD_ERR_OK) {
        ...
        $imagem_antiga = $_SERVER['DOCUMENT_ROOT'] . $portfolio['imagem'];
        if (!empty($portfolio['imagem']) && file_exists($imagem_antiga)) {
            unlink($imagem_antiga);
        }
    }

    $update_sql  = "UPDATE portfolio SET imagem = ?, titulo = ?, descricao_imagem = ? WHERE id = ?";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("sssi", $nova_imagem, $titulo, $descricao_imagem, $portfolio_id);
...
<link rel="stylesheet" href="../css/editar_portfolio.css">
                ...
                <form method="POST"
                    ...
                    <div class="form-group mt-4">
                        ...
                        <input type="text"
                               ...
                               id="titulo"
                               name="titulo"
                               value="<?php echo htmlspecialchars($portfolio['titulo']); ?>">
                    </div>
