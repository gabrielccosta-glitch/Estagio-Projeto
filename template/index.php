                    ...
                    <div>
                        ...
                        <div class="about-contact-card">
                            ...
                            <?php if (!empty($email_principal)): ?>
                                <p><i class="fas fa-envelope"></i> <span><?= htmlspecialchars($email_principal); ?></span></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
...
<div class="lb-backdrop" id="lightbox">
    ...
    <button class="lb-btn prev" id="lbPrev" aria-label="Anterior">
        ...
        <aside class="lb-info" aria-live="polite">
            <p class="lb-kicker">Portf&oacute;lio</p>
...
<script>
    (function() {
        ...
        const cards = Array.from(document.querySelectorAll('.portfolio-card'));
                const images = cards.map((c, index) => {
            const imageEl = c.querySelector('img');
            const title = (c.dataset.title || imageEl.alt || '').trim();
            const description = (c.dataset.description || '').trim();

            return {
                src: imageEl.src,
                alt: imageEl.alt || title || 'Portfolio',
                title: title || `Imagem ${index + 1}`,
        ...
        function update() {
            ...
            if (image.description) {
                lbDescription.textContent = image.description;
                lbDescription.classList.remove('is-empty');
            } else {
                lbDescription.textContent = 'Sem descri\u00e7\u00e3o dispon\u00edvel.';
                lbDescription.classList.add('is-empty');
            }
